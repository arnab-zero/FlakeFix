...
def prepare_input(analysis_path: str) -> dict:
    ...
    print(f"  ✓ Test method: {prepared['test_method']['full_qualified_name']}")
    print(f"  ✓ Hop-1 methods: {hop1_count}")
    print(f"  ✓ Hop-2 methods: {hop2_count}")
...
def run_inlining(
    ...
    if result["success"]:
        print(f"  ✓ Inlining completed successfully")
    else:
        print(f"  ✗ Inlining failed: {result.get('error', 'Unknown error')}")
