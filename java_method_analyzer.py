...
"""
python java_method_extractor.py "C:\Downloads\mercury-main" "C:\Downloads\mercury-main\system\platform-core\src\test\java\org\pla
tformlambda\core\MulticastTest.java" routingTest
"""
...
class JavaMethodAnalyzer:
    ...
    def _index_project(self):
        ...
        print(f"✓ Successfully indexed {indexed_count} Java files")
        if failed_count > 0:
            print(f"✗ Failed to parse {failed_count} files (syntax errors)")
        print(f"✓ Found {len(self.class_to_file)} classes")
        print(f"✓ Indexed {sum(len(methods) for methods in self.method_index.values())} methods")
        print("-" * 80)
    ...
    def print_results(self, results: Dict):
        ...
        if library_methods:
            print(f"\n{'─'*80}")
            print(f"⏭️  LIBRARY METHODS SKIPPED ({len(library_methods)} total)")
            print(f"{'─'*80}\n")
    ...
    def save_results_json(
        ...
        if "error" in results:
            ...
            print(f"\n❌ Saved error JSON to: {output_path}\n")
            return
